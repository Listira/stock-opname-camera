...
with sync_playwright() as p:
    ...
    print("\n=== V2.5: SESSION GALLERY ===")
    check("thumb button populated after saves", "empty" not in (page.get_attribute("#galleryBtn","class") or ""))
    glen = page.evaluate("gallery.length")
    check("gallery has session photos", glen>0, f"{glen} items")
    page.click("#galleryBtn")
    check("gallery modal opens", "open" in page.get_attribute("#galModal","class"))
    cells = page.eval_on_selector_all(".galCell","e=>e.length")
    check("gallery grid rendered", cells>0, f"{cells} cells")
    page.click(".galCell >> nth=0")
    check("lightbox opens on tap", "open" in page.get_attribute("#galView","class"))
    page.click("#galView")
    check("lightbox closes on tap", "open" not in page.get_attribute("#galView","class"))
    page.click("#galClose")
    check("gallery modal closes", "open" not in page.get_attribute("#galModal","class"))
