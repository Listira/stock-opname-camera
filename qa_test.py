...
with sync_playwright() as p:
    ...
    ok=False
    for _ in range(80):
        if "empty" not in (page.get_attribute("#galleryBtn","class") or ""): ok=True; break
        time.sleep(0.1)
    ...
    page.click(".galCell >> nth=0"); page.wait_for_selector("#galView.open", timeout=3000)
    check("lightbox shows full-res blob", page.evaluate("document.querySelector('#galImg').src").startswith("blob:"))
    check("position indicator present", "/" in page.inner_text("#galPos"))
    if n>1:
        b=page.evaluate("viewIdx"); page.evaluate("showView(viewIdx+1)")
        check("navigation (swipe) changes photo", page.evaluate("viewIdx")!=b)
    page.click("#galBack"); time.sleep(0.25)
    check("back button closes lightbox (not app)", "open" not in page.get_attribute("#galView","class"))
    page.click(".galCell >> nth=0"); page.wait_for_selector("#galView.open", timeout=3000)
    nb=page.evaluate("galRecs.length")
    page.click("#galDel"); page.wait_for_selector("#confirmModal.open", timeout=3000)
    check("custom delete confirm (no native popup)", "open" in page.get_attribute("#confirmModal","class"))
    page.click("#confirmYes"); time.sleep(0.4)
    check("delete removes one photo", page.evaluate("galRecs.length")==nb-1, f"{nb} -> {page.evaluate('galRecs.length')}")
    page.evaluate("closeLightbox(); closeGallery(); modalStack=[];")
